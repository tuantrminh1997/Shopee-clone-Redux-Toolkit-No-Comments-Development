...
export default function ShopeeHeaderLogo({ isHeaderForCartLayout }: ShopeeHeaderLogoPropsType) {
	...
	return (
		<PopoverOption
			...
			containerClassName={`${!isHeaderForCartLayout ? "max-w-[202px] max-h-[59px] pr-[40px] lowMobile:hidden" : "min-w-[283.5px] h-[46px]"} `}
			...
			title={
				isHeaderForCartLayout ? (
					<div className='w-full h-full flex items-end justify-start xl:w-[280px]'>
						...
						<span className='text-xl text-[#ee4d2d] ml-2 border-l border-[#ee4d2d] pl-[15px]'>{t("headercartLayout.cart")}</span>
					</div>
				) : (
